...
<body>
    ...
    <div class="p-20 sm:ml-62">
        <div class="flex justify-end mr-12  ">
            <button type="submit" id="openAddCategoryModalBtn" class="bg-blue-300 text-white px-4  py-2 rounded-md hover:bg-blue-300 transition duration-300"> + New Speciality</button>
        </div>
                    ...
                    <thead>
                        <tr>
                            <th class="border-y border-blue-gray-100 bg-blue-gray-50/50 w-64 p-4">
                                <p class="block antialiased font-sans text-sm text-blue-gray-900 font-normal leading-none opacity-70">Specialities</p>
                            </th>
                            <th class="border-y border-blue-gray-100 bg-blue-gray-50/50 w-64 p-4">
                                <p class="block antialiased font-sans text-sm text-blue-gray-900 font-normal leading-none opacity-70">Number of doctors</p>
                            </th>
                        ...
                        <tr>
                            <td class="p-4 border-b border-blue-gray-50">
                                <div class="flex items-center gap-3">
                                    <p class="block antialiased font-sans text-sm leading-normal text-blue-gray-900 font-bold">{{ $speciality->nom }}</p>
                                </div>
                            </td>
                            <td class="p-4 border-b border-blue-gray-50">
                            ...
                            <td class="p-4 border-b border-blue-gray-50">
                                <a href="#" title="Edit" class="editCategorieButton" data-categorie-id="" data-categorie-name="">
                            ...
                            <td class="p-4 border-b border-blue-gray-50">
                                <form action="{{ route('speciality.destroy', ['id' => $speciality->id]) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <a href="{{ route('speciality.destroy', ['id' => $speciality->id]) }}" title="delete">
        ...
        <div id="edit-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
            <div class="bg-white p-6 rounded-lg">
                ...
                <form action="" method="post">
                    ...
                    <label for="editName" class="block mb-2 text-sm font-medium text-gray-900">speciality Name</label>
    ...
    <div id="add-category-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
        <div class="bg-white p-6 rounded-lg">
            @if(session('success'))
            <h2 class="text-2xl font-semibold mb-4">{{ session('success') }}</h2>
            @else
            <h2 class="text-2xl font-semibold mb-4">Add New Speciality</h2>
            <form action="{{ route('speciality.store') }}" method="post">
                @csrf
                <label for="newspeciality" class="block mb-2 text-sm font-medium text-gray-900">Speciality Name</label>
                <input type="text" class="w-full border border-gray-300 p-2.5 rounded-md mb-4" placeholder="Type category name" required="">
                <button type="submit" class="bg-blue-500 text-white rounded-md p-2.5 hover:bg-blue-700">Add speciality</button>
            </form>
            @endif
